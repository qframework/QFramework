...
public class GMath {
    ...
    public static int unProject(float winx, float winy, float winz, 
    ...
    { 
       ...
       mTempData[mTempOffset2]   = (winx - viewport[offsetV]) * 
       						2f / viewport[offsetV+2] - 1.0f; 
       mTempData[mTempOffset2+1] = (winy - viewport[offsetV+1]) * 
       						2f / viewport[offsetV+3] - 1.0f; 
       mTempData[mTempOffset2+2] = 2f * winz - 1.0f; 
       mTempData[mTempOffset2+3] = 1.0f; 

       
       Matrix.multiplyMM(mTempData, mTempOffset1, 
    		   			proj, offsetP, model, offsetM); 
       Matrix.invertM(mTempData, mTempOffset0, 
    		   			mTempData, mTempOffset1); 
       Matrix.multiplyMV(mTempData, mTempOffset3, 
    		   			mTempData, mTempOffset0, 
    		   			mTempData, mTempOffset2); 
       
       if (mTempData[mTempOffset3+3] == 0.0) 
          return GL10.GL_FALSE; 
       
       xyz[offset]  =  mTempData[mTempOffset3  ] / mTempData[mTempOffset3+3]; 
       xyz[offset+1] = mTempData[mTempOffset3+1] / mTempData[mTempOffset3+3]; 
       xyz[offset+2] = mTempData[mTempOffset3+2] / mTempData[mTempOffset3+3]; 
       return GL10.GL_TRUE; 
    } 
		...
		float znear, float zfar , float[] projection)
{
    ...
    public static void lookAtf(float matrix[], float eyePosition[], float center[], float upVector[])
    {
       ...
       Matrix.translateM(matrix, 0, -eyePosition[0], -eyePosition[1], -eyePosition[2]);
