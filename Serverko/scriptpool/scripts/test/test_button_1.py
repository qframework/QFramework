...
def test_script_info() :
	info =  "gname:test_button_1\t"
	info = info + "script:test_button_1.py\t"
	info = info + "players:0\t"
	info = info + "status:0\t"
...
def test_script_userData(userID, data) :
	serverko.trace("JS: test_script_userData " + userID)
	serverko.clientSpectate(userID , 1)
	layout.println("rec-> " + userID + ":" + data )
...
def configureLayout() :
	...
	areaDef10.items = areaDef10.items+ "[i]cards.bela.32,[i]cards.bela.33,[i]cards.bela.34,[i]cards.bela.35,[i]cards.bela.36,[i]cards.bela.37,[i]cards.bela.38,[t]click cards"
	areaDef10.onclick = "clicked"
	areas.append(areaDef10)
	
	
	areaDef1 = LayoutArea()
	areaDef1.type = "text.button"
	areaDef1.id = "text1"
	areaDef1.location="2,2,2,1"
	areaDef1.text = "This is button"
	...
	areaDef2.fields = "start,0,0,3,1,0,1;button,0,3;play,0,1,3,1,0,1;button,1,3;end,0,1,3,1,0,1;button,2,3;none,0,1,3,1,0,1;button,3,3;"
	areaDef2.items = "[i]figures.player.1,[i]figures.player.1,[i]figures.player.3,[t]figures,"
	areaDef2.items = areaDef2.items+ "[i]cards.bela.1,[i]cards.bela.2,[i]cards.bela.3,[t]cards,"
	areaDef2.items = areaDef2.items+ "[i]figures.dice.1,[i]figures.dice.2,[i]figures.dice.4,[t]dices"
	areaDef2.onclick = "clicked"
...
room.script_onUserData.append( test_script_userData )
